...
{
    PTRACE(5, "ALSA\tPSoundChannelALSA0");
  card_nr = 0;
  os_handle = NULL;
  card_nr1 = 0;
  os_handle1 = NULL;
                                            ...
                                            unsigned bitsPerSample)
{
  ...
  PTRACE(5, "ALSA\tPSoundChannelALSA()1");
}
...
{
	PTRACE(5, "ALSA\tUpdateDictionary()");
...
{
	PTRACE(5, "ALSA\tGetDefaultDevice()");
...
PBoolean PSoundChannelALSA::Open_hdmi()
{
    PTRACE(5, "ALSA\tOpen_hdmi "<<"dir:"<<direction<<"numChannels:"<<mNumChannels<<"sampleRate:"<<mSampleRate<<"bitsPerSample:"<<mBitsPerSample);
    PString subdev;
    subdev = "TI81XX XTTX SOUND1 []";
    PStringToOrdinal & devices = playback_devices;
    POrdinalKey * index_hdmi = devices.GetAt(subdev);
    if (index_hdmi == NULL) {
        PTRACE(1, "ALSA\tDevice not found");
        return false;
    }
    subdev = "plughw:"+PString(*index_hdmi);
    card_nr_hdmi = *index_hdmi;

    /* Open in NONBLOCK mode */
    if (snd_pcm_open(&os_handle_hdmi,
                     subdev,
                     SND_PCM_STREAM_PLAYBACK,
                     SND_PCM_NONBLOCK) < 0) {
        PTRACE(1, "ALSA\tOpen subdev 3 Failed");
        return false;
    }
    snd_pcm_nonblock(os_handle_hdmi, 0);
    device_hdmi = subdev;
    enablehdmi = true;
    return true;
}
...
bool PSoundChannelALSA::SetHardwareParams()
{
  ...
  if (direction == Player)
      SetHardwareParams_t(os_handle_hdmi);
  return (SetHardwareParams_t(os_handle)&&SetHardwareParams_t(os_handle1));
}

PBoolean PSoundChannelALSA::SetHardwareParams_t(snd_pcm_t *handle)
{
    if (!handle)
    ...
    for (unsigned retry = 0; retry < 100; ++retry) {
      ...
      if ((err = snd_pcm_hw_params_set_format(handle, hw_params, sndFormat)) < 0) {
        PTRACE(1, "ALSA\tCannot set sample format: " << snd_strerror(err));
        return false;
      }

      if ((err = snd_pcm_hw_params_set_channels(handle, hw_params, mNumChannels)) < 0) {
      ...
      if ((err = snd_pcm_hw_params_set_period_size_near(handle, hw_params, &desiredPeriodSize, &dir)) < 0) {
      ...
      if ((err = (int) snd_pcm_hw_params_set_periods_near(handle, hw_params, &desiredPeriods, &dir)) < 0) {
      ...
      if (((err = snd_pcm_hw_params(handle, hw_params)) >= 0) && initialdual) {
...
PBoolean PSoundChannelALSA::Close_hdmi()
{
    enablehdmi = false;
    if (os_handle_hdmi)
    {
        snd_pcm_close(os_handle_hdmi);
        os_handle_hdmi = NULL;
    }
    return true;
}
...
PBoolean PSoundChannelALSA::Write(const void *buf, PINDEX len)
{
  ...
  if ((!isInitialised && !Setup()) || !len || !os_handle || !os_handle1)
    return false;
  PInt16 *Volbuf = (PInt16 *)buf;
  int Vollen = len/2;
  for (int i=0;i<Vollen;i++)
  {
      *Volbuf = (*Volbuf)*Volratio;
      Volbuf++;
  }

  if (enablehdmi && os_handle_hdmi)
      writetohw(os_handle_hdmi, buf, len, false);
...
{
    PTRACE(5, "ALSA\tGetBuffers "<<size<<"hdmi "<<((!os_handle_hdmi)?"null":"handle"));
   if (size<100) {
       inputdev = size;
   } else if (size==100) {
       Close_hdmi();
...
{
	PTRACE(5, "ALSA\tVolume set "<<set<<" set_vol "<<set_vol);
  int err = 0;
  snd_mixer_t *handle;
