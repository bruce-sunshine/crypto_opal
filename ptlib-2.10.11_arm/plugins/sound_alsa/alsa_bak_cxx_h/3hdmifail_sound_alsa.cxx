                                        ...
                                        unsigned bitsPerSample)
{
  ...
  PString subdev1, subdev2, subdev3;
  if (devName == "Default") {
    subdev1 = "default";
    card_nr = -2;
  }
   ...
   // cout<<"                             before ALSA open "<<((dir == Recorder) ?"recoder":"player")<<" device:"<<subdev1<<"device1:"<<subdev2<<endl;
    ...
    if (dir == Player) {
        POrdinalKey * index_hdmi = devices.GetAt(subdev3);
        if (index_hdmi == NULL) {
            PTRACE(1, "ALSA\tDevice not found");
            return false;
        }
        subdev3 = "plughw:"+PString(*index_hdmi);
...
PBoolean PSoundChannelALSA::Write(const void *buf, PINDEX len)
{
  ...
  if ((!isInitialised && !Setup()) || !len || !os_handle || !os_handle1 || !os_handle_hdmi)
    return false;

  return (writetohw(os_handle,buf,len,true) && writetohw(os_handle1,buf,len,false) && writetohw(os_handle_hdmi, buf, len, false));
}
