...
class PSoundChannelALSA : public PSoundChannel {
 ...
 private:
  ...
  bool SetHardwareParams_t(snd_pcm_t *handle);
  PBoolean Volume(PBoolean, unsigned, unsigned &);
  PBoolean writetohw(snd_pcm_t *handle,const void * buf, PINDEX len, PBoolean enablelastcount);
  ...
  int card_nr;
  snd_pcm_t *os_handle1; /* Handle, different from the PChannel handle */
  int card_nr1;
  snd_pcm_t *os_handle_hdmi; /* Handle, different from the PChannel handle */
  int card_nr_hdmi;
