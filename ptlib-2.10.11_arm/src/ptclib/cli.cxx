...
//dong motify for tcp/telnet control
	WriteString("Welcome to SDT terminal!\n");
  //WritePrompt();
}
...
void PCLI::ProcessDelete(Context &context,PString &commandLine)
{
	context<<left<<setw(81)<<"\r";
	context.flush();
	context<<commandLine;
}

void PCLI::Context::CsMove(int action)
{
	switch (action) {
	  case Left: 
		  *this<<'\033';
...
void PCLI::Context::ArrowKey(int ch)
{
	switch (ch) {
		case e_Up :
			if (m_historyCount == m_commandHistory.rbegin() && m_firstUp){
				m_cli.ProcessDelete(*this,*m_historyCount);
				m_commandLine=*m_historyCount;
				m_xPosition=m_commandLine.GetLength();
				m_firstUp = false;
			}
			else if (m_historyCount != m_commandHistory.begin()){
				m_historyCount--;
				m_cli.ProcessDelete(*this,*m_historyCount);
				m_commandLine=*m_historyCount;
				m_xPosition=m_commandLine.GetLength();
			...
			break;
		case e_Down :
			if (m_historyCount != m_commandHistory.rbegin()){
				m_historyCount++; 
				m_cli.ProcessDelete(*this,*m_historyCount);
				m_commandLine=*m_historyCount;
			} 
		...
		case e_Left :
			...
			break;
		case e_Right :
			if (m_xPosition != m_commandLine.GetLength())
			{
				m_xPosition++;
				CsMove(Right);
			}
...
int PCLI::tabPrint(int CLength, PCLI::Context & context, PString &Cline)
{
  context<<left<<setw(81)<<"\r";
  context.flush();
  PString CommandLine = Cline;
  PString tempCLine = Cline;
  int counts=0;
  ...
  if(CLength == 0){
	...
	for (cmd = m_commands.begin(); cmd != m_commands.end(); ++cmd) {
		int i = cmd->first.Find(CommandLine, 0);
		if(i!= P_MAX_INDEX){
				  counts ++;		  
		  tempCLine = cmd->first;
		  context <<tempCLine<<" ";
  ...
  if (ch != '\n' && ch != '\r') {
    if (m_cli.GetEditCharacters().Find((char)ch) != P_MAX_INDEX) {
      if (!m_commandLine.IsEmpty()) {
		...
		m_xPosition--;
		CsMove(Backspace);
		m_cli.ProcessDelete(*this,m_commandLine);
    ...
    else if (ch > 0 && ch < 256 && isprint(ch) && !m_isKey ) {//ÐÞ¸Ä
	  m_Pcommand=m_commandLine.GetPointer();
	  if (m_xPosition != m_commandLine.GetLength())
	  {
		  m_Pcommand[m_xPosition] = ch;
		  m_xPosition=m_commandLine.GetLength();
		  m_cli.ProcessDelete(*this,m_commandLine);
	  }
	  else{
	...
	else if (ch == 27 || m_isKey) {//ÐÞ
	  if (!m_isKey || ch == 91) {
		  m_isKey = true;
		  return true;
	  }
	  if (m_commandHistory.IsEmpty() && ( ch == 65 || ch==66))
		  return true;
	  ArrowKey(ch);
	}
	else if (ch==9) {
	  m_xPosition=m_cli.tabPrint(m_commandLine.GetLength(), *this, m_commandLine);
	}//¸Ä
...
void PCLI::Context::OnCompletedLine()
{
  ...
  m_historyCount = m_commandHistory.rbegin();
  m_firstUp = true;
}
...
PCLI::PCLI(const char * prompt)
  ...
  , m_prompt(prompt != NULL ? prompt : "")//dong motify for tcp/telnet control
...
void PCLI::ShowHelp(Context & context)//flush
{
  PINDEX i;
  CommandMap_t::const_iterator cmd;
  ...
  for (cmd = m_commands.begin(); cmd != m_commands.end(); ++cmd) {
    if (cmd->second.m_help.IsEmpty() && cmd->second.m_usage.IsEmpty())
	     {context << cmd->first;context.flush();}
    else {
      context << left << setw(maxCommandLength) << cmd->first << "   ";

      if (cmd->second.m_help.IsEmpty())
	     {context << GetCommandUsagePrefix();context.flush();} // Earlier conditon says must have usage
      else {
        lines = cmd->second.m_help.Lines();
        context << lines[0];context.flush();
        for (i = 1; i < lines.GetSize(); ++i)
		context << '\n' << flush << setw(maxCommandLength+3) << ' ' << lines[i];
      }

      lines = cmd->second.m_usage.Lines();
      for (i = 0; i < lines.GetSize(); ++i)
	       context << '\n' << flush << setw(maxCommandLength+5) << ' ' << lines[i];
    }
    context << '\n';context.flush();
  }
