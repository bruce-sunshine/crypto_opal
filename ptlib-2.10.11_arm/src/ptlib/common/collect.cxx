...
PTRACE(2,"PAbstractDictionary::RemoveAt(PINDEX index) 1");
  AbstractSetAt(AbstractGetKeyAt(index), NULL);
PTRACE(2,"PAbstractDictionary::RemoveAt(PINDEX index) 2"); 
  return &obj;
}
...
PBoolean PAbstractDictionary::AbstractSetAt(const PObject & key, PObject * obj)
{
  if (obj == NULL) {
    ...
    if (obj != NULL) {
      if (reference->deleteObjects)
{
PTRACE(2,"PAbstractDictionary::AbstractSetAt 1");
        delete obj;
}
PTRACE(2,"PAbstractDictionary::AbstractSetAt 2");
      reference->size--;
    }
  }
  else {
