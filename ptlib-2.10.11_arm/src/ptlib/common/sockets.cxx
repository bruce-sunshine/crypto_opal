...
{
	PBoolean a= ConvertOSError(os_handle = os_socket(AF_INET, SOCK_STREAM, 0));
	printf("PBoolean PTCPSocket::OpenSocket  %d\n",os_handle);
  return a;
}
...
{
	PBoolean a = ConvertOSError(os_handle = os_socket(ipAdressFamily, SOCK_STREAM, 0));
	printf("PBoolean PTCPSocket::OpenSocket(int ipAdressFamily)  %d\n",os_handle);
  return a;
}
...
PBoolean PUDPSocket::OpenSocketGQOS(int af, int type, int proto)
{
  ...
  printf("PBoolean PUDPSocket::OpenSocketGQOS %d\n",os_handle);

  return ConvertOSError(os_handle);

#else
  PBoolean a = ConvertOSError(os_handle = os_socket(af, type, proto));
  printf("PBoolean PUDPSocket::OpenSocketGQOS %d\n",os_handle);
  return a;
#endif
}
...
#endif
  PBoolean a = ConvertOSError(os_handle = os_socket(AF_INET,SOCK_DGRAM, 0));
  printf("PBoolean PUDPSocket::OpenSocket %d\n",os_handle);
  return a;
}
...
#endif
  PBoolean a = ConvertOSError(os_handle = os_socket(ipAdressFamily,SOCK_DGRAM, 0));
//  int value = 102400*100;
//  socklen_t valSize = sizeof(value);
//  setsockopt(os_handle, SOL_SOCKET, SO_SNDBUFFORCE,(char *)&value, valSize);
//  value = 0;
//  getsockopt(os_handle, SOL_SOCKET, SO_SNDBUF,(char *)&value, &valSize);
//  printf("PBoolean PUDPSocket::OpenSocket(int ipAdressFamily) %d, socket size %d\n",os_handle,value);
//  struct ifreq ifr;
//  memset(&ifr, 0, sizeof(ifr));
//  strncpy(ifr.ifr_name, "eth0", sizeof(ifr.ifr_name));
//  if (-1 == ioctl(os_handle, SIOCGIFTXQLEN, &ifr))
//      printf( "failed to get dev eth0 queue length\n");
//  printf( "Dev eth0 queue length:%d\n ", ifr.ifr_qlen);
//  ifr.ifr_qlen = 1000000;
//  if (-1 == ioctl(os_handle, SIOCSIFTXQLEN, &ifr))
