                                                    ...
                                                    const PString & serviceName) const
{
  ...
  for (PINDEX i = 0; i < m_services.GetSize(); i++) {
    ...
    if (service.serviceType *= serviceType) {
      ...
      {
        PTRACE(4,"sound_alsa.cxx\tCreatePluginsDeviceByName:"<<deviceName<<"dir:"<<userData);
...
{
//ys add for audio control
  PStringArray devices = GetDeviceNames(userData);
  PString tmpdevice = deviceName;
  if (deviceName == "TI81XX XTTX SOUND0 [] #")
      tmpdevice = deviceName.Left(deviceName.GetLength()-2);
  if (
      ((tmpdevice.GetLength() == 2) &&
      (tmpdevice[0] == '#') &&
       isdigit(tmpdevice[1]) &&
       ((tmpdevice[1]-'0') < devices.GetSize()))
      )
    return true;
      
  for (PINDEX j = 0; j < devices.GetSize(); j++) {
    if (devices[j] *= tmpdevice)
      return true;
  }
