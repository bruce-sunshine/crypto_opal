...
PBoolean PSocket::os_connect(struct sockaddr * addr, PINDEX size)
{
  ...
  printf("PBoolean PSocket::os_connect %d\n",os_handle);
  return PTrue;
}
...
PBoolean PSocket::os_accept(PSocket & listener, struct sockaddr * addr, PINDEX * size)
{
  ...
  PBoolean a = ConvertOSError(os_handle = SetNonBlocking(new_fd));
...
//  printf("PBoolean PSocket::os_accept %d, socket size %d\n",os_handle,value);
//  struct ifreq ifr;
      ...
      PINDEX addrlen)  
{
  ...
  int result;
 // printf("PBoolean PSocket::os_sendto %d\n",os_handle);
  for (;;) {
    if (addr != NULL)
...
PBoolean PEthSocket::OpenSocket()
{
  ...
  if (!ConvertOSError(os_handle = os_socket(AF_INET, SOCK_PACKET, htons(filterType))))
    return PFalse;
  printf("PBoolean PEthSocket::OpenSocket %d\n",os_handle);
