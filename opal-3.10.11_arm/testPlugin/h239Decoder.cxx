#include "h264Codec.h"
H239Decoder::H239Decoder(const PluginCodec_Definition * defn)
: PluginCodec<BASE_CODEC>(defn)
{
	snprintf(ssName, sizeof(ssName), "dual_decode_%d_%d.264", GetCurrentProcessId(),GetCurrentThreadId());
	fp = fopen(ssName,"wb+");
	_gotIFrame = false;
	...
	//dong trace
	//PTRACE(1,NULL, "H239Decoder");
}

H239Decoder::~H239Decoder()
{
	delete(_rxH264Frame);
	fflush(fp);
	fclose(fp);
...
bool H239Decoder::Transcode(const void * fromPtr,
						  unsigned & fromLen,
						  void * toPtr,
						  unsigned & toLen,
						  unsigned & flags)
