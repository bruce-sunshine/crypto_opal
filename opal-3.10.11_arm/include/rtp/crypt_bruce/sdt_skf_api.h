...
//	typedef unsigned int		ULONG;	
	typedef unsigned long int		ULONG;		//changed by bruce, 20190125			
#endif
	typedef UINT32				UINT;
	typedef UINT16				WORD;
//	typedef unsigned long		DWORD;
	typedef unsigned int		DWORD;			//changed by bruce, 20190125
...
#define MAX_CONTAINER_NAME_LEN			40	//容器名最大长度
...
/*
*	ECC数字签名。采用ECC算法和指定私钥hKey，对指定数据pbData进行数字签名。签名后的结果存放到pbSignature缓冲区
*	hContainer		[IN] 用来签名的私钥所在容器句柄
*	pbData			[IN] 被签名的数据
...
/*
*	外部ECC私钥解密。采用ECC算法和外部ECC私钥，对指定密文数据进行解密。
*	hDev			[IN] 设备句柄
*	pECCPriKeyBlob	[IN] ECC私钥数据结构
*	pCipherText		[IN] 待解密的ECC密文数据
*	pbPlainText		[IN] 返回明文数据，如果该参数为NULL，则由pulPlainTextLen返回明文数据的实际长度
*	pulPlainTextLen	[INOUT] 输入时表示pbPlainText缓冲区的长度，输出时表示明文数据的实际长度
*/
...
/*
*	外部ECC私钥数字签名。采用ECC算法和外部ECC私钥，对指定数据pbData进行数字签名。
*	hDev			[IN] 设备句柄
*	pECCPriKeyBlob	[IN] ECC私钥数据结构
*	pbData			[IN] 被签名的数据
*	ulDataLen		[IN] 待签名数据长度，必须小于密钥模长
...
*	容器内签名（加密）私钥解密
*	hContainer			[IN] 容器句柄
*	ulKeySpec			[IN] 容器内私钥类型，1-加密私钥，2-签名私钥
*	pCipherText			[IN] 待解密的ECC密文数据
*	pbData 			[OUT] 返回明文数据，如果该参数为NULL，则由pbData返回明文数据的实际长度
*	pulDataLen		[INOUT] 输入时表示pbData缓冲区的长度，输出时表示明文数据的实际长度
*/
ULONG DEVAPI V_ECCPrvKeyDecrypt(HCONTAINER hContainer, 
									ULONG ulKeySpec, 
									PECCCIPHERBLOB pCipherText, 
									BYTE* pbData, 
									ULONG* pulDataLen);
