...
class H323Connection : public OpalRTPConnection
{
  ...
  //@{
	...
	virtual bool startH239Conn();
	bool SendExtendedVideoRequest();
	bool SendGenericIndication();
	void SelectExLogicalChannel(unsigned sessionID);
	//dong change for h239
	virtual bool stopH239Conn();
	bool CloseExtendedVideoRequest();
	bool SendExtendedVideoReleaseCommand();
//dong motify for tcp/telnet control
	virtual bool updateLocalPicture(const unsigned short=0);
	virtual bool updateRemotePicture(const unsigned short=0);
	virtual bool getBandwidth(unsigned* parm, const unsigned short=0);
//dong change for statistic
	virtual bool getStatistic(OpalMediaStatistics&,const unsigned short=0);
