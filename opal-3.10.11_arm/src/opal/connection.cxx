...
OpalConnection::OpalConnection(OpalCall & call,
  ...
  , isDual(false)//dong add for dual notify
  , callToken(token)
  , originating(PFalse)
...
#endif
{
  ...
  //wks mod
  bandwidthAvailable = endpoint.GetInitialBandwidth()*4;
  //wks mod
...
bool OpalConnection::startH239Conn()
{
	return true;
}
//dong change for h239
bool OpalConnection::stopH239Conn()
{
	return true;
}
//dong motify for tcp/telnet control
bool OpalConnection::updateLocalPicture(const unsigned short)
{
	return true;
}
bool OpalConnection::updateRemotePicture(const unsigned short)
{
	return true;
}
bool OpalConnection::getBandwidth(unsigned* parm, const unsigned short)
{
	return true;
}
//dong change for statistic
bool OpalConnection::getStatistic(OpalMediaStatistics &, const unsigned short)
{
	return true;
}
...
void OpalConnection::OnDualEstablished()
{
	PTRACE(3, "OpalCon\t dual OnEstablished " << *this);
	endpoint.OnDualEstablished(*this);
}
...
//dong add for dual notify
  if (GetPhase() == EstablishedPhase&& !isDual  &&stream.GetMediaFormat().GetName() == "H.239") {
	  isDual =true;
	  OnDualEstablished();
  }
  UnlockReadWrite();
...
//dong add for dual notify
  if (isDual  && stream.GetMediaFormat().GetName() == "H.239") {
	  isDual =false;
	  endpoint.OnClosedDualMediaStream(stream);
  }
...
{
  for (OpalMediaStreamPtr mediaStream(mediaStreams, PSafeReference); mediaStream != NULL; ++mediaStream) {
	  if (H323Capability::DefaultVideoSessionID == sessionId || H323Capability::DefaultAudioSessionID==sessionId)
	  { //main video or main audio
		  if (mediaStream->GetSessionID() == sessionId && mediaStream->IsSource() == source)
			  return mediaStream;
	  } 
	  else
	  {//dual video not fixed session Id
		  if ((H323Capability::DefaultVideoSessionID != mediaStream->GetSessionID() 
			  && H323Capability::DefaultAudioSessionID !=mediaStream->GetSessionID())
                                      ...
                                      unsigned requiredBandwidth)
{
  ...
  //wks mod
  if(requiredBandwidth > bandwidthAvailable) {
	  if(endpoint.GetInitialBandwidth()-640>bandwidthAvailable){
		  PTRACE(2, "OpalCon\tAvailable bandwidth exceeded on " << *this);
		  return PFalse;
	  }
	  else
		  requiredBandwidth = endpoint.GetInitialBandwidth()-640;
  }
