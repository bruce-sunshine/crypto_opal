...
PBoolean OpalEndPoint::StartListeners(const PStringArray & listenerAddresses)
{
  ...
  cout<<"interfaces="<<interfaces<<endl; //lee added for test, 20180330
...
void OpalEndPoint::OnDualEstablished(OpalConnection & connection)
{
	manager.OnDualEstablished(connection);
}
...
//dong add for dual notify
void OpalEndPoint::OnClosedDualMediaStream(const OpalMediaStream & stream)
{
	manager.OnClosedDualMediaStream(stream);
}
