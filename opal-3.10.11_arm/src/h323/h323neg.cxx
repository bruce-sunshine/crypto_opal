...
{
PTRACE(2, "~H245Negotiator() 1");
	replyTimer.Stop(false);
PTRACE(2, "~H245Negotiator() 2");
	PThread::Yield();
PTRACE(2, "~H245Negotiator() 3");
...
PTRACE(2, "~H245NegLogicalChannel() 1");
  replyTimer.Stop(false);
  PThread::Yield(); // Do this to avoid possible race condition with timer
PTRACE(2, "~H245NegLogicalChannel() 2");
  delete channel;
PTRACE(2, "~H245NegLogicalChannel() 3");
}
...
PBoolean H245NegLogicalChannel::HandleOpen(const H245_OpenLogicalChannel & pdu)
{
  ...
  //wks add for I frame begin
  PBoolean writeok = PFalse;
  writeok = connection.WriteControlPDU(reply);
  if (ok)
  {
	  state = e_Established;
	  if(channel->GetSessionID() == H323Capability::DefaultVideoSessionID)
	  {
		  //main video apply for I frame
		  H323ControlPDU pdu;
		  pdu.BuildMiscellaneousCommand(channelNumber, H245_MiscellaneousCommand_type::e_videoFastUpdatePicture);
		  connection.WriteControlPDU(pdu);
	  }
	  const H323Capability & capability = channel->GetCapability();
	  //when receive dual video openlogicalchannel
	  if(channel->GetSessionID() == 4 || channel->GetSessionID() == 32 ||capability.GetDefaultSessionID() ==H323Capability::DefaultH239SessionID)
	  {
		  //dual video apply for I frame
		  H323ControlPDU pdu;
		  pdu.BuildMiscellaneousCommand(channelNumber, H245_MiscellaneousCommand_type::e_videoFastUpdatePicture);
		  connection.WriteControlPDU(pdu);
		  //main video apply for I frame
		  H323ControlPDU pduvideo;
		  pduvideo.BuildMiscellaneousCommand(connection.FindChannel(H323Capability::DefaultVideoSessionID,PTrue)->GetNumber(),H245_MiscellaneousCommand_type::e_videoFastUpdatePicture);
		  connection.WriteControlPDU(pduvideo);
	  }
  }
  return writeok;
}
...
PTRACE(2, "H245NegLogicalChannels::RemoveAll() 1");
  channels.RemoveAll();
PTRACE(2, "H245NegLogicalChannels::RemoveAll() 2");
}
