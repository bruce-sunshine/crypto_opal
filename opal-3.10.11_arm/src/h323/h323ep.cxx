...
void H323EndPoint::SetVendorIdentifierInfo(H225_VendorIdentifier & info) const
{
  ...
  info.m_versionId = productInfo.version + " (SDT v" + OpalGetVersion() + ')';
...
PBoolean H323EndPoint::UseGatekeeperAddress(const PString & address)
{
	return UseGatekeeper(address,PString::Empty(),PString::Empty());
}
...
//dong add for gk
void H323EndPoint::OnRegistrationConfirm(PString gkIp)
{
	manager.OnGKRegisterSuccess(gkIp);
}

void H323EndPoint::OnRegistrationReject()
{
	manager.OnGKRegisterFail();
}
...
//dong add for single connection
  int conCout = GetConnectionCount();
  if ( conCout > 0)
  {
...
// Set the cause value
q931PDU.SetCause(Q931::UserBusy);
