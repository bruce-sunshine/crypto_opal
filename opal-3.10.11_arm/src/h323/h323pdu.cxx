...
//ljq
void NotifyPluginVendor(BYTE countryCode)
{
	OpalTranscoderKey opalTransKey("H.264","YUV420P");
	OpalTranscoder * ctrlMsg =OpalTranscoderFactory::CreateInstance(opalTransKey);

	RAVE_CT_CTRL_MSG* param =new RAVE_CT_CTRL_MSG;
	param->cmd=RAVE_CT_ENCODE_MAIN_PROF_SET;
	if (countryCode == 38)
	{
		param->input = PROF_H264HP;
	}else
	{
		param->input = PROF_H264BP;
	}	
	param->output=0;
...
PString H323GetCapabilityIdentifier(const H245_CapabilityIdentifier & capId)
{
  switch (capId.GetTag()) {
    ...
    {
      PString str;
      return PString::Empty();// ys add for keda
