...
PTRACE(2, "H323Connection::OnReleased() 1");
    logicalChannels->RemoveAll();
PTRACE(2, "H323Connection::OnReleased() 2");
    UnlockReadWrite();
  }
  if (endSessionNeeded) {
...
//dong add for remote call rate.
  Q931::InformationTransferCapability capability;
	  unsigned transferRate;
  q931.GetBearerCapabilities(capability,transferRate);
  totalBandwidth = transferRate*64;
  //end
 ...
 if (pdu.m_h323_uu_pdu.m_h323_message_body.GetTag() == H225_H323_UU_PDU_h323_message_body::e_connect)
 {
	 ...
	 H225_AliasAddress  dialledDigits_AliasAddress;
	 PString dialledDigits;
	 PString H323_id;
	 if(connect.m_connectedAddress.GetSize() > 0)
	 {
		 dialledDigits_AliasAddress = connect.m_connectedAddress[0];
		 dialledDigits= H323GetAliasAddressString(dialledDigits_AliasAddress);
		 H323_id = dialledDigits;
	 }	
	 if (connect.m_connectedAddress.GetSize()>1)
	{
			H225_AliasAddress  H323_id_AliasAddress= connect.m_connectedAddress[1];
			H323_id = H323GetAliasAddressString(H323_id_AliasAddress);
	} 	 
	 aliases<<H323_id<<'('<<dialledDigits<<')'<< " [" << remotePartyName << ']';
	 aliases.MakeMinimumSize();
	 remotePartyName =aliases;
}else
{
...
void H323Connection::SetRemoteApplication(const H225_EndpointType & pdu)
{
  if (pdu.HasOptionalField(H225_EndpointType::e_vendor)) {
	...
	//ljq
	NotifyPluginVendor(remoteProductInfo.t35CountryCode);
...
void H323Connection::SetBearerCapabilities(H323SignalPDU & pdu) const
{
  ...
  if (bearerCaps.IsEmpty()) {
	...
	//wks mod
	unsigned transferRate = (GetBandwidthAvailable()/4+639)/640;
	//wks mod
    if (transferRate > 127)
      transferRate = 127;
...
///gsb add
//////////////////////////////////gaoshaobo added for g719:mediaFormat adjust 20160302///////////////////////////////////////

        OpalMediaFormatList localFormatList = localCapabilities.GetMediaFormats();

        PTRACE(3,"localFormatList: ");
        PTRACE(3,localFormatList);
        PString frontFormat = localFormatList.front().GetName();
        //if (!remoteCapabilities.Merge(remoteCaps))//gaoshaobo modified
        if(remoteCaps.FindCapability(frontFormat))
        {
                if (!remoteCapabilities.Merge(localCapabilities))
                        return PFalse;
        }
        else
        {
...
//          return PFalse;
	//////////////////////////////////gaoshaobo added for g719:mediaFormat adjust 20160302///////////////////////////////////////
	remoteCapabilities = remoteCaps; //gsb delete
...
void H323Connection::OnSetLocalCapabilities()
{
  ...
  unsigned int ContentRoleMaskOption;
  for (OpalMediaFormatList::iterator format = formats.begin(); format != formats.end(); ++format) {
	  PString str = format->GetName();
	  ContentRoleMaskOption = format->GetOptionInteger(OpalVideoFormat::ContentRoleMaskOption());
	  if (localCapabilities.FindCapability(format->GetName()) != NULL &&
			format->GetName()== OPAL_H264_H239) {
    /*if (localCapabilities.FindCapability(format->GetName()) != NULL &&
...
OpalMediaStreamPtr H323Connection::OpenMediaStream(const OpalMediaFormat & mediaFormat, unsigned sessionID, bool isSource)
{
  ...
  if (fastStartMediaStream != NULL) {
	...
	/*  for (PINDEX i = 0; i < mediaFormat.GetOptionCount(); i++) {
		  ...
		  PString str = option.GetName();
						

		  if(str == PString("Generic Parameter 3"))
		  {  
//			  ((OpalMediaOptionUnsigned &)option).SetValue(resolution.customMaxMBPS);
			  cout << "Generic Parameter 3" << endl;		
			  cout << &option << endl;
		  }
		  if(str == PString("Generic Parameter 4"))
		  {
//			  ((OpalMediaOptionUnsigned &)option).SetValue(resolution.customMaxFS);
			  cout << "Generic Parameter 4" << endl;		
			  cout << &option << endl;
		  }
		  if(str == PString("Generic Parameter 41"))
		  {
//			  ((OpalMediaOptionUnsigned &)option).SetValue(resolution.profile);
			  cout << "Generic Parameter 41" << endl;		
			  cout << &option << endl;
		  }

		  if(str == PString("Generic Parameter 42"))
		  {
//			  ((OpalMediaOptionUnsigned &)option).SetValue(resolution.level);
			  cout << "Generic Parameter 42" << endl;		
			  cout << &option << endl;
		  }
	  }
//end ljq */
...
//dong change for h239
#define OpalPluginCodec_Identifer_H239 "0.0.8.239.2"
bool H323Connection::startH239Conn()
{
	//SendExtendedVideoRequest(); //dong keda may send this msg, but not necessary
	SendGenericIndication();
	SelectExLogicalChannel(H323Capability::DefaultH239SessionID);
	return true;
}
bool H323Connection::stopH239Conn()
{
	CloseExtendedVideoRequest();
	SendExtendedVideoReleaseCommand();
	return true;
}
//dong motify for tcp/telnet control
bool H323Connection::updateLocalPicture(const unsigned short channelId)
{
	OpalMediaStreamPtr mediaStream;
	if (channelId==1)
	{
		mediaStream = GetMediaStream(H323Capability::DefaultH239SessionID,TRUE);
	}else
	{
		mediaStream = GetMediaStream(H323Capability::DefaultVideoSessionID,TRUE);
	}
	if (mediaStream !=NULL)
		mediaStream->ExecuteCommand(OpalVideoUpdatePicture());
	return true;
}
bool H323Connection::updateRemotePicture(const unsigned short channelId)
{
	OpalMediaStreamPtr mediaStream;
	if (channelId==1)
	{
...
bool H323Connection::getBandwidth(unsigned* parm, const unsigned short channelId)
{
	OpalMediaStreamPtr mediaStream;
	if (channelId==1)
	{
	...
	if (mediaStream !=NULL)
	{
		/*OpalGetBandWidth opalGetBandWidth;
		mediaStream->ExecuteCommand(opalGetBandWidth);
		unsigned* bandwidth =static_cast<unsigned*>(opalGetBandWidth.GetPlugInData());
		parm[0] = bandwidth[0];
		parm[1] = bandwidth[1];*/
	}
	return true;
...
//dong change for statistic
bool H323Connection::getStatistic(OpalMediaStatistics& statistics,const unsigned short channelId)
{
	OpalMediaStreamPtr mediaStream;
	switch (channelId)
	{
	case 1:
		mediaStream = GetMediaStream(H323Capability::DefaultAudioSessionID,FALSE);
		break;
	case 2:
		mediaStream = GetMediaStream(H323Capability::DefaultAudioSessionID,TRUE);
		break;
	case 3:
		mediaStream = GetMediaStream(H323Capability::DefaultVideoSessionID,FALSE);
		break;
	case 4:
		mediaStream = GetMediaStream(H323Capability::DefaultVideoSessionID,TRUE);
		break;
	case 5:
		mediaStream = GetMediaStream(H323Capability::DefaultH239SessionID,FALSE);
		break;
	case 6:
		mediaStream = GetMediaStream(H323Capability::DefaultH239SessionID,TRUE);
		break;
	...
	if (mediaStream !=NULL)
		mediaStream->GetStatistics(statistics, false);
	return true;
}
//dong change for h239
//dong: genericRequest
bool H323Connection::CloseExtendedVideoRequest()
{

	H323Channel * pChannel = this->FindChannel(H323Capability::DefaultH239SessionID, TRUE);
	if (pChannel == NULL)
		pChannel = this->FindChannel(H323Capability::DefaultH239SessionID, FALSE);

	if(pChannel != NULL && pChannel->IsOpen())
	{
		CloseLogicalChannelNumber(pChannel->GetNumber());
	}
	//return true;
	//dong: end by return;
	H323ControlPDU reply;

	H245_GenericMessage &msg = reply.Build (H245_RequestMessage::e_genericRequest); 
	...
	PASN_ObjectId &object_id = id;
	object_id = OpalPluginCodec_Identifer_H239;
	...
	H245_GenericParameter & param0 = content[0];
	H245_ParameterIdentifier & idm0 = param0.m_parameterIdentifier; 
	idm0.SetTag(H245_ParameterIdentifier::e_standard);
	PASN_Integer & idx0 = idm0;
	idx0 = 44;
	H245_ParameterValue & genvalue0 = param0.m_parameterValue;
	genvalue0.SetTag(H245_ParameterValue::e_unsignedMin);
	PASN_Integer & valg0 = param0.m_parameterValue;
	valg0 =0;//257

	H245_GenericParameter & param1 = content[1];
	H245_ParameterIdentifier & idm1 = param1.m_parameterIdentifier; 
	idm1.SetTag(H245_ParameterIdentifier::e_standard);
	PASN_Integer & idx1 = idm1;
	idx1 = 42;
	H245_ParameterValue & genvalue1 = param1.m_parameterValue;
	genvalue1.SetTag(H245_ParameterValue::e_unsignedMin);
	PASN_Integer & valg1 = param1.m_parameterValue;
	valg1 =8;//19


	H245_GenericParameter & param2 = content[2];
	H245_ParameterIdentifier & idm2 = param2.m_parameterIdentifier; 
	idm2.SetTag(H245_ParameterIdentifier::e_standard);
	PASN_Integer & idx2 = idm2;
	idx2 = 43;
	H245_ParameterValue & genvalue2 = param2.m_parameterValue;
	genvalue2.SetTag(H245_ParameterValue::e_unsignedMin);
	PASN_Integer & valg2 = param2.m_parameterValue;
	valg2 = 88;//0
	return WriteControlPDU(reply);
}
//dong: genericCommand
bool H323Connection::SendExtendedVideoReleaseCommand()
{
	H323ControlPDU reply;
	H245_GenericMessage &msg = reply.Build (H245_CommandMessage::e_genericCommand); 
...
bool H323Connection::SendExtendedVideoRequest()
{
	H323ControlPDU reply;
	H245_GenericMessage &msg = reply.Build (H245_RequestMessage::e_genericRequest); 
	msg.IncludeOptionalField(H245_GenericMessage::e_subMessageIdentifier);
...
//dong change for h239
bool H323Connection::SendGenericIndication()
{
	H323ControlPDU reply;

	H245_IndicationMessage &msg1 = reply.Build (H245_IndicationMessage::e_genericIndication); 
	H245_GenericMessage  & msg = msg1;
	msg.IncludeOptionalField(H245_GenericMessage::e_subMessageIdentifier);
	msg.IncludeOptionalField(H245_GenericMessage::e_messageContent);
	...
	genvalue0.SetTag(H245_ParameterValue::e_unsignedMin);
	PASN_Integer & valg0 = param0.m_parameterValue;
	valg0 =0;//257; //tandberg //terminalLabel
	//valg0 =0; //cisco
	...
	genvalue1.SetTag(H245_ParameterValue::e_unsignedMin);
	PASN_Integer & valg1 = param1.m_parameterValue;
	valg1 =4;//6;//tandberg  //channelId
	//valg1 =4; //cisco

	return 	WriteControlPDU(reply);
}
//dong change for h239
void H323Connection::SelectExLogicalChannel(unsigned sessionID)
{
	...
	for (PINDEX i = 0; i < localCapabilities.GetSize(); i++) {
		...
		{
			if (OpenLogicalChannel(localCapability,sessionID, H323Channel::IsTransmitter))
				break;
		}
	}
}
