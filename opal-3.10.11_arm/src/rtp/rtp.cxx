...
PBoolean RTP_Session::inited = PFalse;
unsigned char* RTP_Session::key = (unsigned char*)defKey;

/***********gaoshaobo for srtp********/
#endif

#if bruce
PBoolean RTP_Session::audio_zrtp_inited = PFalse;
int RTP_Session::audio_zrtp=0;
extern int zrtp_use_audio;
#endif
...
#if gaoshaobo
  , created(false) //gaoshaobo for srtp
  , createdIn(false)//gaoshaobo for srtp
#endif 
{
...
RTP_Session::~RTP_Session()
{
  ...
  ////////////////////////////////////////////////
    ...
    if(RTP_Session::audio_zrtp_inited && RTP_Session::audio_zrtp)
    {
		...
		int error_num=0;
		printf("222\n");
//		if((error_num=zrtp_stream_stop(stream_audio))!=zrtp_status_ok)
//			printf("zrtp_stream_stop audio error , error = %d\n", error_num);
		printf("333\n");
		zrtp_session_down(session_audio);
		printf("444\n");
		zrtp_down(zrtp_audio);
		printf("555\n");
		RTP_Session::audio_zrtp_inited = FALSE;
		dataMutex.Signal();
		printf("666\n");
    }
#endif
...
{
	if(!RTP_Session::audio_zrtp)		//added by lee, for zrtp
	return EncodingLock(*this)->OnSendData(frame);
#if gaoshaobo
	/************gaoshaobo for srtp*********************/

	srtp_err_status_t err;
	SendReceiveStatus stat = EncodingLock(*this)->OnSendData(frame);
	...
	if(!created&&inited)
	{
		...
		policyOut.ssrc.value = frame.GetSyncSource();
		policyOut.key = (unsigned char*)inputKey;
		policyOut.ekt = NULL;
		policyOut.next = NULL;
		policyOut.window_size = 300;
		policyOut.allow_repeat_tx = 0;
		policyOut.rtp.sec_serv =(srtp_sec_serv_t)( sec_serv_none | sec_serv_conf_and_auth);
		policyOut.rtcp.sec_serv = sec_serv_none;

		int iKeyLen = policyOut.rtp.cipher_key_len*2;
		...
		if(created)
		{
			  ...
			  err = ::srtp_protect(ctxOut, frame.GetPointer(), &len);
			  if (err != srtp_err_status_ok)
			  {
				  printf("\n\n\n\n*****************\n\n"
				  								"\n\nsrtp protected failed Out\n\n\n\n\n\n\n\n");
	...
	/************gaoshaobo for srtp*********************/
		...
		if(RTP_Session::audio_zrtp)
		{
			...
			if(RTP_Session::audio_zrtp_inited && RTP_Session::audio_zrtp)
			{
				frame.SetExtensionSizeDWORDs(0);
//				frame.SetExtension(true);		//set extension 1
				
				unsigned int len=frame.GetHeaderSize() + frame.GetPayloadSize();
//				printf("OnSendData_1,GetHeaderSize=%d, len=%d\n",frame.GetHeaderSize(),len);
				status_audio = zrtp_process_rtp(stream_audio, (char*)frame.GetPointer(), (unsigned int*)&len);
				if(status_audio != zrtp_status_ok)
				{
					printf("Error:\t zrtp process rtp error, code : %d\n", status_audio);
					return RTP_Session::e_IgnorePacket;
				}
				frame.SetPayloadSize(len - frame.GetHeaderSize());
//				printf("OnSendData_2,GetHeaderSize=%d, len=%d\n",frame.GetHeaderSize(),len);
			}
			return e_ProcessPacket;
		}
#endif
...
RTP_Session::SendReceiveStatus RTP_Session::Internal_OnSendData(RTP_DataFrame & frame)
{
  ...
  {
	  //dong rtp send rate
	  //PTRACE(1, "WritePacket\tsend RTP frame"<<markerSendCount);
    markerSendCount++;
  }
...
RTP_Session::SendReceiveStatus RTP_Session::OnReceiveData(RTP_DataFrame & frame)
{
	...
	if(!createdIn&&inited)
	{
		memset(&policyIn, 0, sizeof(policyIn));
		...
		policyIn.ssrc.value = frame.GetSyncSource();
		policyIn.key = (unsigned char*)inputKey;//key;
		policyIn.ekt = NULL;
		policyIn.next = NULL;
		policyIn.window_size = 300;
		policyIn.allow_repeat_tx = 0;
		policyIn.rtp.sec_serv = (srtp_sec_serv_t)( sec_serv_none | sec_serv_conf_and_auth);
		policyIn.rtcp.sec_serv = sec_serv_none;


		int iKeyLen = policyIn.rtp.cipher_key_len*2;
		int keyLen = hex_string_to_octet_string(inputKey, defKey, iKeyLen);
		if(keyLen < iKeyLen)
		{
			return e_IgnorePacket;
	...
	if(createdIn)
	{
		  ...
		  err = ::srtp_unprotect(ctxIn, frame.GetPointer(), &len);

		  if (err != srtp_err_status_ok)
		  {
			  printf("\n\n\n\n*****************\n\n"
			  				  								"\n\nsrtp unprotected failed IN %d, frame len : %d\n\n\n\n\n\n\n\n", err, len);
			  return RTP_Session::e_IgnorePacket;
		  }
		  frame.SetPayloadSize(len - frame.GetHeaderSize());
	}
...
#if bruce
//	zrtp_stream_info_t info;	//gaoshaobo for zrtp
	if(RTP_Session::audio_zrtp_inited && RTP_Session::audio_zrtp)
	{
//		frame.SetExtensionSizeDWORDs(0);
		
		unsigned int len = frame.GetHeaderSize() + frame.GetPayloadSize();
		status_audio = zrtp_process_srtp(stream_audio, (char*)frame.GetPointer(), (unsigned int*)&len);
//		printf("OnReceiveData_1,GetHeaderSize=%d,len=%d\n",frame.GetHeaderSize(),len);
		if(status_audio != zrtp_status_ok)
		{
			printf("Error:\t zrtp process srtp error, code : %d\n", status_audio);
		}
//		printf("OnReceiveData_2,GetHeaderSize=%d,len=%d\n",frame.GetHeaderSize(),len);
		frame.SetPayloadSize(len - frame.GetHeaderSize());
	}

#endif

//  printf("RTP_Session::SendReceiveStatus, len=%d\n",frame.GetHeaderSize() + frame.GetPayloadSize());
...
#if bruce
class RTP_UDP;
//PUDPSocket * RTP_UDP::sock_audio=NULL;
RTP_UDP* RTP_UDP::rtp_udp_audio=NULL;
...
#if bruce
//  zrtp_recv_thread=NULL;
#endif
}

void* RTP_UDP::zrtp_audio_stop(void*)
{
	pthread_detach(pthread_self());
	printf("stop zrtp audio thread\n");
	if(RTP_Session::audio_zrtp_inited && RTP_Session::audio_zrtp)
    	{
		RTP_UDP::rtp_udp_audio=NULL;
		RTP_Session::audio_zrtp_inited = FALSE;
		printf("111\n");
//		get_obj()->dataMutex.Wait();
		int error_num=0;
		printf("222\n");
		error_num=zrtp_stream_stop(stream_audio);
		printf("check zrtp_stream_stop audio wheather has errors , return = %d\n", error_num);
		printf("333\n");
		zrtp_session_down(session_audio);
		printf("444\n");
		zrtp_down(zrtp_audio);
		printf("555\n");
//		get_obj()->dataMutex.Signal();
		printf("666\n");
...
#if bruce
/*
  PAssert(zrtp_recv_thread->WaitForTermination(10000), "zrtp_recv_thread did not terminate");
  delete zrtp_recv_thread;
  zrtp_recv_thread=NULL;

*/

  if(RTP_Session::audio_zrtp_inited && RTP_Session::audio_zrtp && isAudio)
  {
//    	pthread_create(&thread_audio_stop, NULL, zrtp_audio_stop, NULL);
	RTP_UDP::rtp_udp_audio=NULL;
	RTP_Session::audio_zrtp_inited = FALSE;
  }
...
//dong change for h239 //dong MCU may send QOS, just for backup
PQoS & RTP_UDP::GetQOS()
{
	if (dataSocket != NULL) 
...
#if bruce
/*
int RTP_UDP::zrtp_sender(const zrtp_stream_t * stream_audio, char* packet, unsigned int length)
{
//	PWaitAndSignal mutex(zrtp_mutex);
	
	zrtp_stream_t* tmpStr = (zrtp_stream_t*)stream_audio;
	int lenSend;
	status_audio = zrtp_process_rtp(tmpStr, packet, &length);
	if(status_audio != zrtp_status_ok)
	{
		printf("[zrtp_main_sender] :\t Error when process rtp packet(s)");
		return status_audio;
	}

	PBoolean send_ok=sock_audio->Write(packet,length);
//	lenSend = sendto(sock->os_handle, packet, length, 0, (struct sockaddr*)&sendaddr_audio, sizeof(struct sockaddr_in));
//	if(lenSend<=0)
	if(!send_ok)
	{
		printf("[zrtp_main_sender] :\t Error when send rtp packet(s)");
//		return 1;
	}
...
RTP_UDP * RTP_UDP::get_obj()
{
	return RTP_UDP::rtp_udp_audio;
}

int RTP_UDP::zrtp_sender_get(const zrtp_stream_t * stream_audio, char* packet, unsigned int length)
{

//	PWaitAndSignal mutex(get_obj()->dataMutex);
	...
	PBoolean send_ok=get_obj()->dataSocket->Write(packet,length);
//	lenSend = sendto(sock->os_handle, packet, length, 0, (struct sockaddr*)&sendaddr_audio, sizeof(struct sockaddr_in));
//	if(lenSend<=0)
	if(!send_ok)
	{
...
/*
void* RTP_UDP::zrtp_recv(void*)
{
	pthread_detach(pthread_self());
//	PWaitAndSignal mutex(zrtp_mutex);
	int lenRecv;
	PBoolean zrtpOk = false;
	int sockLen = sizeof(struct sockaddr);
	char recvBuf[200];
	zrtp_stream_info_t info;
	unsigned int recv_len=200;

	while(!zrtpOk)
	{
//		lenRecv = recvfrom(sock->os_handle, recvBuf, 200, 0, (struct sockaddr*)&recvaddr_audio, (unsigned int*)&sockLen);
//		zrtp_mutex.Wait();
		PBoolean recv_ok=sock_audio->Read(recvBuf,recv_len);
//		printf("recvBuf=%s\n",recvBuf);
		unsigned int read_len=sock_audio->GetLastReadCount();
//		zrtp_mutex.Signal();
//		if(lenRecv>0)
		if(recv_ok)
		{
//			status_audio = zrtp_process_srtp(stream_audio, recvBuf, (unsigned int*)&lenRecv);
			status_audio = zrtp_process_srtp(stream_audio, recvBuf, (unsigned int*)&(read_len));
			zrtp_stream_get(stream_audio, &info);
			if(info.state == ZRTP_STATE_SECURE)
			{
				zrtpOk = true;
			}			
		}
...
//		lenRecv = recvfrom(sock->os_handle, recvBuf, 200, 0, (struct sockaddr*)&recvaddr_audio, (unsigned int*)&sockLen);
		dataMutex.Wait();
		PBoolean recv_ok=dataSocket->Read(recvBuf,recv_len);
//		printf("recvBuf=%s\n",recvBuf);
		unsigned int read_len=dataSocket->GetLastReadCount();
		dataMutex.Signal();
//		if(lenRecv>0)
		if(recv_ok)
		{
...
static void* callback_zrtp_recv( void* arg )  
{  
    ((RTP_UDP*)arg)->zrtp_recv();
    return NULL;  
} 

/*
void  RTP_UDP::Recv_Thread(PThread &, INT)
{
//	pthread_detach(pthread_self());
	PWaitAndSignal mutex(zrtp_mutex);
//	PSafeLockReadWrite safeLock(sock_audio);
	int lenRecv;
	PBoolean zrtpOk = false;
	int sockLen = sizeof(struct sockaddr);
	char recvBuf[200];
...
#if bruce
   if(isAudio && !remoteTransmitAddress.IsValid())
      {
      		printf("SetRemoteSocketInfo_2, localAddress=%s, remoteAddress=%s, remoteTransmitAddress=%s,localDataPort=%d, remoteDataPort=%d\n",
			localAddress.AsString().GetPointer(0), 
			remoteAddress.AsString().GetPointer(0),
			remoteTransmitAddress.AsString().GetPointer(0),
			localDataPort,
			remoteDataPort);
	
//      sock_audio=dataSocket;
      RTP_UDP::rtp_udp_audio=this;
      unsigned int Remote_IP_Net=(unsigned int)(inet_addr(remoteAddress.AsString().GetPointer(0))+2);
      printf("zrtp_use_audio=%d\n",zrtp_use_audio);	
      if(zrtp_use_audio)
      {	
    	  RTP_Session::audio_zrtp=zrtp_use_audio;
      }	
      else
      {
    	  RTP_Session::audio_zrtp=0;
      }
      printf("RTP_Session::audio_zrtp=%d,RTP_Session::audio_zrtp_inited=%d\n",RTP_Session::audio_zrtp, RTP_Session::audio_zrtp_inited);
	if(!RTP_Session::audio_zrtp_inited && RTP_Session::audio_zrtp)
	{
			
		printf("audio zrtp init\n");
		zrtp_config_defaults(&config_audio);
		memcpy(config_audio.zid, &Remote_IP_Net, sizeof(Remote_IP_Net));
		config_audio.lic_mode = ZRTP_LICENSE_MODE_UNLIMITED;
		config_audio.cb.misc_cb.on_send_packet = zrtp_sender_get;
		memset(config_audio.cache_file_cfg.cache_path,0,sizeof(config_audio.cache_file_cfg.cache_path));
		strncpy(config_audio.cache_file_cfg.cache_path,"./zrtp_def_cache_path_audio.dat",31) ;
		status_audio = zrtp_init(&config_audio, &zrtp_audio);
		if(status_audio != zrtp_status_ok)
		{
			printf("[ZRTP_Error]:\t zrtp init erro, code : %d\n", status_audio);
			exit(status_audio);
		}
		zrtp_profile_defaults(&profile_audio, zrtp_audio);
		profile_audio.discovery_optimization = 0;
		status_audio = zrtp_profile_check(&profile_audio, zrtp_audio);
		if(status_audio != zrtp_status_ok)
		{
			printf("[ZRTP_Error]:\t zrtp profile check erro, code : %d\n", status_audio);
			exit(status_audio);
		}
		status_audio = zrtp_session_init(zrtp_audio, &profile_audio, ZRTP_SIGNALING_ROLE_UNKNOWN, &session_audio);
		if(status_audio != zrtp_status_ok)
		{
			printf("[ZRTP_Error]:\t zrtp session init error, code : %d\n", status_audio);
			exit(status_audio);
		}
		status_audio = zrtp_stream_attach(session_audio, &stream_audio);
		if(status_audio != zrtp_status_ok)
		{
			printf("[ZRTP_Error]:\t zrtp stream attach error, code : %d\n", status_audio);
			exit(status_audio);
		}
		//begin zrtp stream
		status_audio = zrtp_stream_start(stream_audio, 11);
		if(status_audio != zrtp_status_ok)
		{
			printf("[ZRTP_Error]:\t zrtp stream start error, code : %d\n", status_audio);
			exit(status_audio);
		}
		//create a thread for zrtp packet receiving
//		zrtp_thread_create(zrtp_recv, NULL);
//		dataMutex.Wait();
		pthread_create(&thread_recv_zrtp_audio, NULL, callback_zrtp_recv, this);
//		dataMutex.Signal();
//  		zrtp_mutex.Wait();

// 		if (zrtp_recv_thread == NULL) {
//   			 zrtp_recv_thread = PThread::Create(PCREATE_NOTIFIER(Recv_Thread), "ZRTP RECV");
//zrtp_recv_thread = PThread::Create(PCREATE_NOTIFIER(Recv_Thread), 0, PThread::AutoDeleteThread, PThread::NormalPriority, "ZRTP RECV", 65535*16);
//    			 zrtp_recv_thread->Resume();
//  		}
...
//  		zrtp_mutex.Signal();
		...
		RTP_Session::audio_zrtp_inited = TRUE;
	    }
	}
...
#if 0
//	zrtp_stream_info_t info;	//gaoshaobo for zrtp
	if(RTP_UDP::audio_zrtp_inited && RTP_UDP::audio_zrtp)
	{
		frame.SetExtensionSizeDWORDs(0);
		unsigned int len1 = frame.GetHeaderSize() + frame.GetPayloadSize();
printf("ReadData_1, GetHeaderSize=%d, len1=%d, dataSocket->GetLastReadCount()=%d\n",frame.GetHeaderSize(), len1, dataSocket->GetLastReadCount());
		unsigned int len = dataSocket->GetLastReadCount();
		printf("ReadData_1,GetHeaderSize=%d,len=%d\n",frame.GetHeaderSize(), len);
		status_audio = zrtp_process_srtp(stream_audio, (char*)frame.GetPointer(), (unsigned int*)&len);
		if(status_audio != zrtp_status_ok)
		{
			printf("Error:\t zrtp process srtp error, code : %d\n", status_audio);
		}
		printf("ReadData_2,GetHeaderSize=%d,len=%d\n",frame.GetHeaderSize(), len);
//		frame.m_headerSize-=4;
		frame.SetPayloadSize(len - frame.GetHeaderSize());
	}
...
{
//  if(!RTP_UDP::audio_zrtp)		//added by lee, for zrtp, bruce
  return EncodingLock(*this)->WriteData(frame, false);
#if 0
		
		if(RTP_UDP::audio_zrtp)
		{
/*
			PBoolean stat = EncodingLock(*this)->WriteData(frame, false);
			if (!stat)
			{
				printf("Error: EncodingLock(*this)->WriteData(frame, false)\n");
				return false;
			}
*/
			if(RTP_UDP::audio_zrtp_inited && RTP_UDP::audio_zrtp)
...
//				frame.SetExtension(true);		//set extension 1
				unsigned int len=frame.GetHeaderSize() + frame.GetPayloadSize();
				printf("WriteData_1,GetHeaderSize=%d,len=%d\n",frame.GetHeaderSize(),len);
				status_audio = zrtp_process_rtp(stream_audio, (char*)frame.GetPointer(), (unsigned int*)&len);
				if(status_audio != zrtp_status_ok)
				{
					printf("Error:\t zrtp process rtp error, code : %d\n", status_audio);
				...
				frame.SetPayloadSize(len - frame.GetHeaderSize());
				printf("WriteData_2,GetHeaderSize=%d,len=%d\n",frame.GetHeaderSize(),len);
			}
