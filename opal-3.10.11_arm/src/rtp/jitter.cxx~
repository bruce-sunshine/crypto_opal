...
void OpalJitterBufferThread::WaitForThreadTermination()
{
  ...
  if (jitterThread != NULL) {
    ...
    PAssert(jitterThread->WaitForTermination(10000), "Jitter buffer thread did not terminate");	//note, changed by lee,10000->100000
    delete jitterThread;
  } 
}
...
void OpalJitterBufferThread::JitterThreadMain(PThread &, INT)
{
  ...
  startjitterrecord = 0;
  jitterrecord = fopen("/home/root/record/jitterrecord.g711","wb");
...
//     Keep reading from the RTP transport frames
    if (!OnReadPacket(frame) || !WriteData(frame))
      m_running = false;
    if(startjitterrecord == 1 && jitterrecord != NULL)
    {
    	BYTE * outputbytes = frame.GetPayloadPtr();
        int outputsize = frame.GetPayloadSize();
        fwrite((char *)outputbytes,1,outputsize,jitterrecord);
    }
  }
