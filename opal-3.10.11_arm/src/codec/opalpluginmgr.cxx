...
//dong change for deorder
  if ((flags & PluginCodec_ReturnCoderRequestIFrame) != 0 ||
          (m_lastSequenceNumber != UINT_MAX && (m_lastSequenceNumber+1) < sequenceNumber) /*(m_lastSequenceNumber+1) != sequenceNumber)*/&&(m_lastSequenceNumber!=65535)) {
  ...
  if ((flags & PluginCodec_ReturnCoderLastFrame) == 0)
    return true;
  m_totalFrames++;//dong change for statistic
return true; //add by ck
  ...
  for (unsigned  i = 0; i < count; i++,codecDefn++) {
    ...
    switch (codecDefn->flags & PluginCodec_MediaTypeMask) {
      ...
      default:
        ...
        cout<< "OpalPlugin\tno media transcoder factory created for codec " << codecDefn->descr<<endl;
        continue;
    }
